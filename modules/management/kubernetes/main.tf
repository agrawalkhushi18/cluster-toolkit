...
# Creates the kubeconfig file dynamically during the apply phase.
resource "local_file" "kubeconfig" {
  ...
  filename = "${path.module}/kubeconfig.gke.yaml"
}

# Configures the official Kubernetes provider, deferred until apply time.
provider "kubernetes" {
  alias       = "in_cluster"
...
resource "kubernetes_manifest" "this" {
  ...
  provider = kubernetes.in_cluster
