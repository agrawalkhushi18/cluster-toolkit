...
variable "cluster_dependency" {
  description = "A resource from the cluster to depend on, ensuring it's created before this module runs."
  type        = any
}

variable "cluster_endpoint" {
  description = "The endpoint for the GKE cluster's API server."
  type        = string
}
...
variable "cluster_token" {
  description = "A valid authentication token for the GKE cluster."
  type        = string
  sensitive   = true
}
